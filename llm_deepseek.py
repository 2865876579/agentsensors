...
# ── 睡眠报告回调（由 main.py 注册）──────────────────────
_sleep_report_cb = None  # async def cb(client_id: str, turn_id: int) -> str

def set_sleep_report_callback(cb):
    global _sleep_report_cb
    _sleep_report_cb = cb
...
- ★ 用户要操作电脑时必须调用对应 PC 工具，并根据工具返回结果确认是否真的完成，不准光嘴上答应。打开应用用 pc_open_app；点击当前页面或窗口内的可见文字用 pc_click_ui；创建 Word/Excel/PPT 用 pc_office_task；已有文件发 App 用 pc_send_file_to_app；发微信联系人用 pc_send_wechat_file；截图用 pc_screenshot。参数已完整时直接执行，不要再次确认
- ★ 你可以调用 read_sensors 工具查看枕头传感器状态（压力分布/温湿度/光照/空气质量）。用户问"枕头现在怎样""温度多少"或想了解睡姿压力时使用
- ★ 用户询问昨晚/最近一次睡眠质量、睡眠时长、鼾声次数或体动情况，必须调用 get_sleep_report。报告由云端生成并发送到手机App，不需要电脑在线
...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "pc_open_app",
            "description": (
                "在用户电脑启动、恢复并置顶应用窗口。用户明确要求打开 WPS、微信或浏览器时必须调用。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "app": {"type": "string", "description": "应用名：WPS、微信、浏览器，或可执行文件绝对路径"},
                    "url": {"type": "string", "description": "打开浏览器时可选的网址"},
                },
                "required": ["app"],
            },
        },
    },
    {
        ...
        "function": {
            "name": "pc_click_ui",
            "description": (
                "在当前前台应用窗口中按可见文字定位并点击控件、菜单、列表项或链接。"
                "用户说点开、点击、选择页面内某项时必须调用；只提供目标文字，不猜屏幕坐标。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "target": {"type": "string", "description": "界面上可见的目标文字，例如四梁八柱"},
                    "app": {"type": "string", "description": "可选；需要先置顶的应用名，例如WPS、微信或浏览器"},
                },
                "required": ["target"],
            },
        },
    },
    {
        ...
        "function": {
            "name": "pc_office_task",
            "description": (
                "在桌面创建 Word、Excel 或 PPT，校验文件并用 WPS 在前台打开；还可在同一次任务中发送到 App 或微信。"
                "用户已经给出文件类型、内容和发送目标时必须立即调用，不要再次询问。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "kind": {"type": "string", "enum": ["word", "spreadsheet", "presentation"]},
                    "filename": {"type": "string", "description": "带扩展名的桌面文件名"},
                    "title": {"type": "string"},
                    "content": {"type": "string", "description": "Word正文或PPT正文；支持用#表示标题、-表示列表"},
                    "headers": {"type": "array", "items": {"type": "string"}, "description": "Excel表头"},
                    "rows": {"type": "array", "items": {"type": "array", "items": {}}, "description": "Excel数据行"},
                    "slides": {
                        "type": "array",
                        "items": {
                            "type": "object",
                    ...
                    "send_to_app": {"type": "boolean", "description": "用户要求发送到App时必须为true"},
                    "wechat_contact": {"type": "string", "description": "用户要求发微信时填写精确联系人名称"},
                    "message": {"type": "string", "description": "发送微信文件时附带的可选文字"},
                },
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "pc_send_file_to_app",
            "description": "把电脑上的已有文件上传到小安 App。用户要求把文件发到 App 时必须调用。",
            "parameters": {
                "type": "object",
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "pc_send_wechat_file",
            "description": "把电脑上的已有文件发送给名称完全匹配的微信联系人。",
            "parameters": {
                "type": "object",
                "properties": {
                    "contact": {"type": "string", "description": "微信联系人精确名称"},
    ...
    },
    {
        ...
        "function": {
            "name": "pc_screenshot",
            "description": "截取电脑主屏幕并把图片发送到小安 App。用户要求截图发到 App 时必须调用。",
    ...
    },
    {
        ...
        "function": {
            "name": "pc_open_file",
            "description": "打开电脑上已经存在的文件并验证窗口。不能用它创建新文件。",
            "parameters": {
                "type": "object",
                "properties": {"path": {"type": "string", "description": "已存在文件的绝对路径"}},
                "required": ["path"],
            },
        },
    },
    {
        ...
        "function": {
            ...
            "description": "在桌面创建纯文本文件；Word、Excel、PPT必须使用pc_office_task。",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            "name": "pc_run_command",
            "description": "在电脑执行系统命令并返回真实退出码和输出。仅在其他专用PC工具不适用时调用。",
            "parameters": {
                "type": "object",
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "汇总最近一次由FSR压力确认有人时记录的睡眠数据，包含有效在床时长、"
                "心率、呼吸、R60体动、鼾声次数和具体时间，并把云端生成的报告文件发送到手机App。"
                "用户问昨晚睡得怎么样、最近睡眠质量、鼾声或要求睡眠报告时必须调用。"
                "此工具完全在云端运行，不使用PC Agent。"
            ),
            "parameters": {"type": "object", "properties": {}},
        },
    },
...
def _forced_pc_tool(user_text: str) -> str | None:
    compact = re.sub(r"\s+", "", str(user_text or "")).lower()
    office_nouns = ("word", "docx", "excel", "xlsx", "ppt", "pptx", "文档", "表格", "演示文稿")
    create_verbs = ("创建", "生成", "制作", "新建", "写一份", "做一份", "整理成")
    if any(noun in compact for noun in office_nouns) and any(verb in compact for verb in create_verbs):
        return "pc_office_task"
    if "截图" in compact and any(word in compact for word in ("发到app", "发送到app", "传到app", "给app")):
        return "pc_screenshot"
    if any(word in compact for word in ("打开wps", "打开微信", "打开浏览器", "启动wps", "启动微信", "启动浏览器")):
        return "pc_open_app"
    return None
...
async def _prepare_forced_pc_arguments(tool_name: str, user_text: str) -> dict:
    compact = re.sub(r"\s+", "", user_text).lower()
    if tool_name == "pc_screenshot":
        return {}
    if tool_name == "pc_open_app":
        if any(name in compact for name in ("wps", "word", "文档")):
            return {"app": "WPS"}
        if "微信" in compact:
            return {"app": "微信"}
        return {"app": "浏览器"}

    prompt = f"""你是电脑 Office 任务参数生成器。只输出一个 JSON 对象，不要解释。
字段：
- kind: word/spreadsheet/presentation
- filename: 带 docx/xlsx/pptx 扩展名的文件名
- title: 文档标题
- content: Word正文或PPT正文；需要整理资料时写出完整、可直接交付的内容
- headers: Excel表头数组
- rows: Excel二维数据数组
- slides: PPT页面数组，每项含title/content
- send_to_app: 用户要求发送到App时为true
- wechat_contact: 用户要求发微信时填精确联系人，否则空字符串
- message: 微信附言，否则空字符串
保留用户明确指定的文件名、标题和正文。若用户要求搜索或整理资料，利用联网信息生成可交付内容。
...
用户任务：{user_text}"""
    ...
    arguments = _parse_json_object(response.choices[0].message.content or "")
    extension_match = re.search(r"([A-Za-z0-9][A-Za-z0-9_. -]{0,100}\.(?:docx|xlsx|pptx))", user_text, re.IGNORECASE)
    if extension_match:
        arguments["filename"] = extension_match.group(1).strip()
    filename = str(arguments.get("filename") or "").strip()
    suffix = Path(filename).suffix.lower()
    if suffix == ".xlsx" or "excel" in compact or "表格" in compact:
        arguments["kind"] = "spreadsheet"
        arguments.setdefault("filename", "整理表格.xlsx")
    elif suffix == ".pptx" or "ppt" in compact or "演示文稿" in compact:
        arguments["kind"] = "presentation"
        arguments.setdefault("filename", "整理演示.pptx")
    else:
        arguments["kind"] = "word"
        arguments.setdefault("filename", "整理资料.docx")
    if "app" in compact:
        arguments["send_to_app"] = any(word in compact for word in ("发送", "发到", "传到", "给app"))
    return arguments


def _forced_pc_reply(tool_name: str, arguments: dict, result: str) -> str:
    if any(word in result for word in ("失败", "缺少", "不存在", "超时", "未成功")):
        return result
    if tool_name == "pc_office_task":
        parts = ["文件已经生成，并在 WPS 前台打开。"]
        if arguments.get("send_to_app"):
            parts.append("也已经发送到 App。")
        if arguments.get("wechat_contact"):
            parts.append(f"已经发送给微信联系人{arguments['wechat_contact']}。")
        return "".join(parts)
    if tool_name == "pc_screenshot":
        return "截图已经发送到 App。"
    if tool_name == "pc_open_app":
        return f"{arguments.get('app', '应用')}已经打开并放到电脑最前面。"
    return result


async def _dispatch_pc_action(action: str, params: dict, client_id: str, turn_id: int) -> str:
    guard = guard_ai_action("pc_agent", action=action)
    if not guard["allowed"]:
        return guard["reason"]
    print(f"[Tool] pc action={action!r} params={params!r}")
    if not _pc_command_cb:
        return "电脑助手未连接，暂无法执行此操作。"
    try:
        result = await _pc_command_cb(action, params, client_id, turn_id)
        print(f"[Tool] pc result={result[:100]!r}")
        return result
    except Exception as e:
        print(f"[Tool] pc error: {e}")
        return f"执行失败：{e}"
...
async def _dispatch_tool(name: str, arguments: dict, *, client_id: str = "", turn_id: int = 0) -> str:
    ...
    if name == "get_weather":
        ...
        action = {
            "pc_open_app": "run_app",
            "pc_click_ui": "click_text",
            "pc_office_task": "create_office_file",
            "pc_send_file_to_app": "send_file_to_app",
            "pc_send_wechat_file": "send_wechat_file",
            "pc_screenshot": "screenshot",
            "pc_open_file": "open_file",
            "pc_write_text_file": "desktop_write",
            "pc_run_command": "run_cmd",
        }[name]
        return await _dispatch_pc_action(action, arguments, client_id, turn_id)
        ...
        if isinstance(params, str):
            try:
                ...
                params = {}
        return await _dispatch_pc_action(action, params, client_id, turn_id)
    ...
    elif name == "get_sleep_report":
        print(f"[Tool] get_sleep_report client={client_id} turn={turn_id}")
        if not _sleep_report_cb:
            return "睡眠报告服务暂未连接。"
        try:
            return await _sleep_report_cb(client_id, turn_id)
        except Exception as e:
            print(f"[Tool] get_sleep_report error: {e}")
            return f"生成睡眠报告失败：{e}"
...
async def chat_stream(user_text: str, history: list[dict] | None = None,
    ...
    forced_tool = _forced_pc_tool(user_text)
    if forced_tool:
        try:
            arguments = await _prepare_forced_pc_arguments(forced_tool, user_text)
            ...
            reply = _forced_pc_reply(forced_tool, arguments, result)
        except Exception as exc:
            reply = f"电脑任务执行失败：{exc}"
